        ...
        fun (_Room, FromUser, Message) ->
            ...
            case Message of 
                {tictactoe, UserName, Action} ->
                    case Action of 
                        {update, State} ->
                            python:call(P, 'chat_client', 'receive_state', [list_to_binary(FromUser), State]);
                        {start, State} ->
                            python:call(P, 'chat_client', 'receive_start', [list_to_binary(FromUser), State]);
                        over ->
                            python:call(P, 'chat_client', 'receive_game_over', [list_to_binary(FromUser)]) end;
                [_X | _Xs] -> python:call(P, 'chat_client', 'receive_chat_default', [list_to_binary(Message), list_to_binary(FromUser)]);
                _ -> ok end
        end,
        UserName),
    {ok, P}.
